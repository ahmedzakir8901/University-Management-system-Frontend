// src/pages/Finance/tabs/InvoicesTab.jsx
...
import { financeService } from '../../../services/financeService';              // 3 dots
import InvoiceFormModal from '../../../components/finance/InvoiceFormModal';      // 3 dots
import PaymentFormModal from '../../../components/finance/PaymentFormModal';      // 3 dots
import { formatCurrency } from '../../../utils/formatCurrency';                  // 3 dots
import { formatDate } from '../../../utils/formatDate';                          // 3 dots
import RoleGate from '../../../components/RoleGate';                             // 3 dots
import { useAuth } from '../../../context/AuthContext';                          // 3 dots
import ExportButton from '../../../components/common/ExportButton';              // 3 dots

function InvoicesTab() {  // <-- RENAMED
  const { user } = useAuth();
  ...
  return (
    <Box>
      {/* Header row: Search on left, buttons on right (title moved to parent page) */}
            ...
            {filteredInvoices.length === 0 ? (
              ...
              filteredInvoices.map((invoice) => (
                <TableRow key={invoice.id} hover>
                  ...
                  <TableCell>
                    {invoice.status !== 'PAID' && (
                      <IconButton color="primary" onClick={() => handleMakePayment(invoice)} title="Make Payment" size="small">
                        <Payment />
                      </IconButton>
                    )}
                    <RoleGate allowedRoles={['ADMIN']}>
                      <IconButton color="error" onClick={() => handleDelete(invoice.id)} title="Delete" size="small">
                        <Delete />
                      </IconButton>
                    </RoleGate>
                  </TableCell>
